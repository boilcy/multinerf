...
def log_delta(path, c2w) -> np.ndarray:
    Rot = c2w[..., :3, :3]
    T = c2w[..., :3, 3]
...
def create_train_step(model: models.Model,
  ...
  def train_step(
    ...
    def loss_fn(variables, poseVar):
      c2ws = poseModel.apply(poseVar, cameras[1])
      _cameras = (cameras[0], c2ws, cameras[2],cameras[3])
      rays = batch.rays
      if config.cast_rays_in_train_step:
        rays = camera_utils.cast_ray_batch(_cameras, rays, camtype, xnp=jnp)
      ...
      stats['weight_l2s'] = train_utils.summarize_tree(variables['params'], train_utils.tree_norm_sq)
    ...
    return new_state, stats, new_poseState, rng
...
def main(unused_argv):
  ...
  rng, key = random.split(rng)
  poseModel, poseState, pose_lr_fn = inerf_helper.setup_union_optim(config, rng, cameras[1].shape[0])
  ...
  for step, batch in zip(range(init_step, num_steps + 1), pdataset):
    ...
    state, stats, poseState, rngs = train_pstep(
        rngs,
        state,
        poseState,
        batch,
    ...
    if step % config.print_every == 0:
      v = jax.pmap(
        lambda x, y: poseModel.apply(x.params, y), 
        in_axes=(0, None), 
        axis_name='batch')(poseState, cameras[1])
      log_delta(log_dir, v[0])
